...
void read_file(int socket_fd, unsigned int offset, unsigned int size) {
    printf("Read %d bytes (0x%x) starting at offset %d\n", size, size, offset);
...
void process_request(int socket_fd, unsigned char *addr, unsigned int size) {
    unsigned int offset;
    unsigned char op;
...
void sawa_listen(int socket_fd) {
    int n, size=0;
...
int get_fs_file() {
    ...
    printf("Filesystem file does not exist, creating it...\n");
